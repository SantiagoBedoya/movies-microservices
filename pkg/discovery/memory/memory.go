...
func NewRegistryWithServiceAddrs(serviceAddrs map[string][]string) *Registry {
	results := make(map[string]map[string]*serviceInstance)
	for k, v := range serviceAddrs {
		results[k] = map[string]*serviceInstance{}
		for _, h := range v {
			results[k][k] = &serviceInstance{hostPort: h, lastActive: time.Now()}
		}
	}
	return &Registry{
		serviceAddrs: results,
	}
}
